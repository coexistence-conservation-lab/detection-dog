...
pacman::p_load(broom, caret, emmeans, ggbeeswarm2, plotrix, janitor, pROC, pwr, readxl, tidyverse)
```
...
dog_summary <- data.frame(
  ...
  time = search$mean,
  efficiency  = (dog_matrix$byClass[1]/search$mean)*1000,
  row.names = NULL
)
...
human_summary <- human_matrix %>%
  summarise(
    ...
    time = mean(human_results$search_time_s),
    efficiency  = (sensitivity/mean(human_results$search_time_s))*1000
  )
