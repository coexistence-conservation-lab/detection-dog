...
How many trials needed to determine dog performs better than sensitivity 0.5 (chance). 

```{r}
# Function to calculate required sample size for proportion test
dog_sensitivity_power <- function(
  target_sensitivity = 0.8,   # Expected sensitivity (true positive rate)
  null_sensitivity = 0.5,     # Null hypothesis value (typically 0.5 for chance)
  power = 0.8,               # Desired power (typically 0.8)
  alpha = 0.05,              # Significance level
  alternative = "greater"    # Usually one-sided test for sensitivity
) {
...
ggplot(scenarios, 
  ...
  labs(title = "Required Sample Size for Detection Dog Sensitivity Testing",
    subtitle = "Compared to sensitivity 0.5 (chance)",
...
compare_sensitivity_power <- function(
  ...
  power = 0.8,              # Desired power (typically 0.8)
  alpha = 0.05,             # Significance level
  alternative = "greater"   # Usually one-sided test for dog > human
) {
